...
# 动态绘图
def plot(uav_cluster_x, uav_cluster_y, uav_cluster_z, ue_cluster, bs_cluster, time_array, target_slice_array, building_cluster, reward, episode, steps):
    ...
    for rows in uav_cluster_x:
        ...
        while True:
            ...
            # 动态 delay 折线图
            if step_index >= 32 and step_index <= len(time_array[rows_index]) + 31:
                # 设置x轴坐标
                xx.append(time_array[rows_index][step_index - 32])
                # 设置y轴坐标
                yy.append(target_slice_array[rows_index][step_index - 32])
